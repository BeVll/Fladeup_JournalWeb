...
export const ListTeachers = () => {
    ...
    const getItems= (page:number, pageSize:number, filterValue: string, column:Key | undefined , direction:string | undefined ) => {
        ...
        TeacherApi.getAllTeachers(page, pageSize, filterValue, column, direction).then(res => {
            setItems(res.data);
    ...
    const onDeleteItem = () => {
        if(deleteItem){
            TeacherApi.deleteTeacher(deleteItem?.id).then(res => {
                console.log(res.data);
                setOpenDelete(false);
                setRefresh(true);
            })
        ...
        items ?
            <>
                ...
                <CustomTable
                    ...
                    onOpenChange={() => {navigate("/teachers/create")}}
                    tableBody={
                        <TableBody emptyContent={!isLoading ? "No teachers found" : <></>} loadingContent={<Spinner/>} isLoading={isLoading} items={isLoading ? [] : items.data}>
                        {(item: IStudentModel) => (
                            <TableRow key={item.id}>
                                ...
                                <TableCell>
                                    <div className="grid items-center gap-2 grid-cols-2"
                                        ...
                                        <Link href={"teachers/view/"+item.id} color={"foreground"} underline={"hover"}>
                                ...
                                <TableCell>
                                    <Chip variant={"flat"} color={item.status.toLowerCase() == "teacher" ? "primary" : "danger"}>
                                        {item.status}
                                    </Chip>
                                </TableCell>
                                <TableCell>
                                        ...
                                        <Dropdown>
                                            ...
                                            <DropdownMenu variant="faded" aria-label="Dropdown menu with icons">
                                                <DropdownItem
                                                    ...
                                                    shortcut="⌘N"
                                                    onPress={() => {
                                                        navigate("/teachers/view/"+item.id);
                                                    }}
                                                ...
                                                <DropdownItem
                                                    ...
                                                    onPress={() => {
                                                        navigate("/teachers/edit/"+item.id);
                                                    }}
                                                >
                                                    Edit
                                                </DropdownItem>
